...
*/
__global__ void my_bev_pool_v2_kernel(int c, int n_intervals,
...
__global__ void my_bev_pool_v2_set_zero_kernel(int n_points, float* __restrict__ out) {
  int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void my_bev_pool_v2(int c, int n_intervals, const float* depth, const float* feat, const int* ranks_depth,
  const int* ranks_feat, const int* ranks_bev, const int* interval_starts, const int* interval_lengths, float* out,
  cudaStream_t stream) {
  my_bev_pool_v2_kernel<<<(int)ceil(((double)n_intervals * c / 256)), 256, 0, stream>>>(
...
void my_bev_pool_v2_set_zero(int n_points, float* out) {
  my_bev_pool_v2_set_zero_kernel<<<(int)ceil(((double)n_points / 256)), 256>>>(n_points, out);
}