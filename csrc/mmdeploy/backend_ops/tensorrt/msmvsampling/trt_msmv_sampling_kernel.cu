...
__global__ void msmv_sampling_set_zero_kernel(int n_points, float* __restrict__ out) {
...
void ms_deformable_im2col_cuda_c2345(
    ...
    ms_deformable_im2col_gpu_kernel_c2345 <<<GET_BLOCKS(num_kernels, num_threads), num_threads, 0, stream>>> (
...
void msmv_sampling_set_zero(int n_points, float* out) {
    // const int num_kernels = batch_size * num_query * channels;
    const int num_threads = CUDA_NUM_THREADS;
    msmv_sampling_set_zero_kernel<<<GET_BLOCKS(n_points, num_threads), num_threads>>>(n_points, out);
}